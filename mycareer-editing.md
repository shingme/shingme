...
- `strengths.cases`: 세 가지 강점 사례와 단계별 내용
  - 각 단계의 `description`은 문자열 배열이며, 배열 항목마다 화면에 불릿으로 표시됩니다.
  - 수행 업무를 추가하려면 `description` 배열에 문자열을 추가합니다.
...
4. GROWTH의 네 역량은 처음에 전체 너비 막대로 표시됩니다. 각 막대를 클릭하면 왼쪽으로 줄어들면서 AI 활용 영역과 직접 책임질 판단이 열리고, 다시 클릭하면 닫힙니다.
5. 기반 강점과 성장 역량은 제목이 나란히 표시되며, 한쪽을 열면 다른 쪽은 자동으로 닫힙니다. 열린 제목을 다시 누르면 초기 상태로 접힙니다.
